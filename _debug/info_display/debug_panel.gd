...
func _ready() -> void:
	visible = gm.show_debug
	if gm.show_logic_map:
		logic_map.show_map()
	else:
		logic_map.hide_map()
...
func _input(event) -> void:
	if visible and event.is_action_pressed("L_key"):
		gm.show_logic_map = !gm.show_logic_map
		if gm.show_logic_map:
			logic_map.show_map()
		else:
			logic_map.hide_map()
		...
		visible = !visible
		gm.show_debug = !gm.show_debug
		if gm.show_logic_map and !visible:
			gm.show_logic_map = false
			logic_map.hide_map()
...
func _process(delta) -> void:
	if visible:
		...
		property_show_logic_map.text = "show_logic_map: "+str(gm.show_logic_map)
